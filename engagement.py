...
for influencer in influencers:
    for tweet in tweepy.Cursor(api.search_tweets, q=f'to:{influencer}',result_type='recent').items(200):
        p = done_qt_likes.fetch({"value": tweet.id_str,"lang":"en"})
        if p.count != 0:
            continue
        try:
        ...
        done_qt_likes.put({"value": tweet.id_str,"lang":"en"})
        done=1
        break
    if done ==1:
        break
...
for user in api.get_followers(id ='22256645',count=200):
    p = done_qt_users.fetch({"value": user.screen_name,"lang":"en"})
    if p.count != 0:
		    continue
    try:
        user.follow()
    ...
    done_qt_users.put({"value": user.screen_name,"lang":"en"})
    break
