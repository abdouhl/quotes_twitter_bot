...
quote_kkey = random.choice(range(0,2990))
data =supabase.table("quotes").select('*').eq('lang','es').range(quote_kkey,quote_kkey+1).execute().data
title = data[0]['text']
author_name = data[0]['username']
quote_key = data[0]['key']
auth = author_name.lower()

site_url = "https://www.quotesandsayings.net/es/quotes/"+author_name

tweet = f'"{title[:120]}"-@{auth}\n\n#{auth} #citas #citasyrefranes #refranes #frases {site_url}-{quote_key}'
